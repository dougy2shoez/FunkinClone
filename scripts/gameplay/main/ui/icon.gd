...
func _process(delta: float) -> void:
	if createdIcons:
		...
		if lastBeat != get_parent().get_parent().get_node("Conductor").currBeat:
			$enemyIcon.scale.x += 0.1
			$enemyIcon.scale.y += 0.1
			$playerIcon.scale.x += -0.1
			$playerIcon.scale.y += 0.1
...
func setIconBopTween(delta: float):
	$playerIcon.scale.x += (-1 - $playerIcon.scale.x) / (0.15 / delta)
	$playerIcon.scale.y += (1 - $playerIcon.scale.y) / (0.15 / delta)
	$enemyIcon.scale.x += (1 - $enemyIcon.scale.x) / (0.15 / delta)
	$enemyIcon.scale.y += (1 - $enemyIcon.scale.y) / (0.15 / delta)
